...
export default function ClipInOne() {
  ...
  const handleFiles = (fileList) => {
    const incoming = Array.from(fileList).slice(0, 8 - images.length);
    const withUrls = incoming.map((file) => ({
      file,
      url: URL.createObjectURL(file),
      id: `${file.name}-${Date.now()}-${Math.random()}`,
    }));
    setImages((prev) => [...prev, ...withUrls].slice(0, 8));
  };
  ...
  const missing = [];
  if (images.length < 3) missing.push("至少上传3张图片");
            ...
            <div className="grid grid-cols-4 gap-2">
              {images.map((img) => (
                <div
                  ...
                  style={{ height: "72px", background: "#00000010" }}
                >
                  <img
                    src={img.url}
                    alt=""
